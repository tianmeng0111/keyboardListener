...
/**
 * Created by Tian on 2018/2/26.
 */

public class AdjustSizeLinearLayout extends LinearLayout {

    private static final String TAG = "AdjustSizeLinearLayout";
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        Log.e(TAG, "change" + w + " " + h + " " + oldw + " " + oldh);
        if (oldw == 0 || oldh == 0)
            return;
        if (boardListener != null) {
            if (oldw != 0 && h - oldh < -mChangeSize) {
                Log.e(TAG,"键盘弹出"+ "change" + w + " " + h + " " + oldw + " " + oldh);
                boardListener.keyBoardVisable(Math.abs(h - oldh));
            }
            if (oldw != 0 && h - oldh > mChangeSize) {
                Log.e(TAG,"键盘结束"+ "change" + w + " " + h + " " + oldw + " " + oldh);
